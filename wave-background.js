...
function renderWaveBackground(container, settings) {
  ...
  const waveColor = getSystemColorScheme() === "light"
    ? "rgba(37, 37, 37, 0.08)"
...
async function initialize() {
  ...
  renderWaveBackground(container, currentSettings);
  onSettingsChanged((settings) => {
    currentSettings = settings;
    renderWaveBackground(container, settings);
  });

  window.matchMedia?.("(prefers-color-scheme: dark)").addEventListener("change", () => {
    if (currentSettings) renderWaveBackground(container, currentSettings);
  });
}
